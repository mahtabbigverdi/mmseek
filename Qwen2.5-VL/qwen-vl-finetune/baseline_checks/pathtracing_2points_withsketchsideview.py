...
def save_evals(model, processor ,model_id):
    ...
    for item in tqdm(val_data):
        ...
        side_view_image = image_path.replace("topdown", "pathtracing_sketch")
        messages = [{
        "role": "user",
        "content": [
            {"type": "image", "image": image_path},
            {"type": "text", "text": prompt},
            {"type": "text", "text": f"Hint: A sketch-style abstract drawing from the first-person perspective at position M1, looking {side}, is provided below:"},
            # {"type": "text", "text": f"Hint: The depth map of the first-person view of the {side} side at M1 is shown below:"},
    ...
    save_path = f"outputs/{model_id}_path_tracing_2point_answers_sketchsideview.json"
