...
def save_evals(model, processor ,model_id):
    # val_data_path  =  "/mmfs1/gscratch/krishna/mahtab/mmseek/Data/validation_perspective_depth.json"
    val_data_path  =  "/mmfs1/gscratch/krishna/mahtab/mmseek/Data/validation_habitat_depth.json"
    ...
    for item in val_data:
        ...
        image_path = image_path.replace("input/", "thought/")
        print(image_path)
        prompt = "What is the depth map of this image?"
        # prompt = item['conversations'][0]['value'].replace('<image>\n', '')
        print(prompt)
        # prompt += "\nplease put the final answer in a boxed{}."
        output = eval(model, processor, prompt, image_path, max_new_tokens=2048)
        answers[index] = output
        print(f"Index: {index}")
    save_path = f"outputs/perspective/{model_id}_habitat_perspective_answers.json"
