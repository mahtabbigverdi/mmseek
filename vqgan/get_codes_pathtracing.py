...
class VQGANProcessor:
    ...
    def preprocess_image(self, image_path, resize=True):
        ...
        img = img.convert('RGB')  # Convert grayscale to RGB by duplicating channels
...
if __name__ == "__main__":
    ...
    # reconstructed = processor.reconstruct_image('/mmfs1/gscratch/krishna/mahtab/mmseek/Data/path_tracing/2points/validation/sideview/1.png')
    # reconstructed.save('test_reconstruction.png')
    
    # # Encode image to indices
    # print("\nEncoding to indices...")
    # indices, h, w = processor.encode_to_indices('/mmfs1/gscratch/krishna/mahtab/mmseek/Data/path_tracing/2points/validation/sideview/1.png')
    ...
    for img in tqdm(os.listdir('/mmfs1/gscratch/krishna/mahtab/mmseek/Data/path_tracing/2points/depthmaps')):
        if img in output:
            continue
        indices, h, w = processor.encode_to_indices(f'/mmfs1/gscratch/krishna/mahtab/mmseek/Data/path_tracing/2points/depthmaps/{img}')

        output[img] = np.array(indices).flatten().tolist()
        assert len(output[img]) == 256
        # if len(output) % 100 == 0:
        #     with open(f'/mmfs1/gscratch/krishna/mahtab/mmseek/vqgan/train_depth_2points_sideview_pathtracing_codes_vqgan_f16_1024.json', 'w') as f:
        #         json.dump(output, f)
    with open(f'/mmfs1/gscratch/krishna/mahtab/mmseek/vqgan/train_depth_2points_sideview_pathtracing_codes_vqgan_f16_1024.json', 'w') as f:
        json.dump(output, f, indent=4)
    # Test reconstruction quality first
    # print("\nTesting direct reconstruction...")
    # reconstructed = processor.reconstruct_image('/mmfs1/gscratch/krishna/mahtab/mmseek/Data/path_tracing/2points/sideview/1.png')
    # reconstructed.save('test_reconstruction.png')
    
    # # Encode image to indices
    # print("\nEncoding to indices...")
    # indices, h, w = processor.encode_to_indices('/mmfs1/gscratch/krishna/mahtab/mmseek/Data/path_tracing/2points/sideview/1.png')
    # print(f"Encoded to {h}×{w} grid ({h*w} total indices)")
