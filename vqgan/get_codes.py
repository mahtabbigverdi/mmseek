...
#         config_path='/mmfs1/gscratch/krishna/mahtab/mmseek/vqgan/vqgan_gumbel_f8_8192/configs/model.yaml',
#         checkpoint_path='/mmfs1/gscratch/krishna/mahtab/mmseek/vqgan/vqgan_gumbel_f8_8192/ckpts/last.ckpt',
...
#     for depth_f in tqdm(os.listdir('/mmfs1/gscratch/krishna/mahtab/syntheic_data/Depth-Anything/ADE_depth/')[20200:]):
#         if not depth_f.endswith('.png'):
#             continue
#         indices, h, w = processor.encode_to_indices(f'/mmfs1/gscratch/krishna/mahtab/syntheic_data/Depth-Anything/ADE_depth/{depth_f}')

#         output[depth_f] = np.array(indices).flatten().tolist()
#         assert len(output[depth_f]) == 1024
#         if len(output) % 100 == 0:
#             with open(f'/mmfs1/gscratch/krishna/mahtab/mmseek/vqgan/depth_codes_ade_vqgan8192.json', 'w') as f:
#                 json.dump(output, f)
#     with open(f'/mmfs1/gscratch/krishna/mahtab/mmseek/vqgan/depth_codes_ade_vqgan8192.json', 'w') as f:
#         json.dump(output, f)
...
if __name__ == "__main__":
    ...
    output = {}
    for depth_f in tqdm(os.listdir('/mmfs1/gscratch/krishna/mahtab/depth/only_depth')):
        if not depth_f.endswith('.png'):
            continue
        indices, h, w = processor.encode_to_indices(f'/mmfs1/gscratch/krishna/mahtab/depth/only_depth/{depth_f}')

        temp = np.array(indices).flatten().tolist()
        output[depth_f] = "<DEPTH_START>" +"".join([f"<DEPTH_{x}>" for x in temp]) +"<DEPTH_END>"
        # assert len(output[depth_f]) == 1024
        if len(output) % 100 == 0:
            with open(f'/mmfs1/gscratch/krishna/mahtab/mmseek/vqgan/depth_codes_blink_vqgan8192.json', 'w') as f:
                json.dump(output, f)
    with open(f'/mmfs1/gscratch/krishna/mahtab/mmseek/vqgan/depth_codes_blink_vqgan8192.json', 'w') as f:
        json.dump(output, f)
    # # Test reconstruction quality first
    # print("\nTesting direct reconstruction...")
    # reconstructed = processor.reconstruct_image('/mmfs1/gscratch/krishna/mahtab/syntheic_data/Depth-Anything/ADE_depth/ADE_train_00000001_depth.png')
    # reconstructed.save('test_reconstruction.png')
    
    # # Encode image to indices
    # print("\nEncoding to indices...")
    # indices, h, w = processor.encode_to_indices('/mmfs1/gscratch/krishna/mahtab/syntheic_data/Depth-Anything/ADE_depth/ADE_train_00000001_depth.png')
    # print(f"Encoded to {h}×{w} grid ({h*w} total indices)")
    # 
    # # Decode indices back to image
    # print("\nDecoding from indices...")
    # reconstructed_img = processor.decode_from_indices(indices)
    # reconstructed_img.save('reconstructed_from_indices.png')
    
    # # Get codebook embeddings
    # embeddings = processor.get_codebook_embeddings()
    # np.save('codebook_embeddings-8k.npy', embeddings.cpu().numpy())
    # print(f"\nCodebook shape: {embeddings.shape}")
